...
const ProfileColumn = () => {
  ...
  const orderHistory = myProducts.map((product, ind, arr) => {
    return (
      <div className={styles['order-item']} key={ind}>
        <div className={styles['order-info']}>
            ...
            <div className={styles['order-details']}>
                <h4>{product.name}</h4>
                {ind + 1 === arr.length && <p className={styles['new-item']}>New</p>}
            </div>
            <button onClick={() => history.push(`/product/${product.productCode}`)}>
  ...
  return (
    <div className={styles["column-cont"]}>
      <div className={styles["cont-text"]}>
        <h2 className={styles["cont-budget"]}>$ {user.budget}</h2>
        <p className={styles["cont-date"]}>{dateFormat}</p>
      </div>
      <div className={styles["cont-orders"]}>
        <h3>Order History</h3>
        <div className={styles["order-column"]}>
            {orderHistory}
        </div>
      </div>
    </div>
